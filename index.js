...
var sqlTool = require('./sqltest.js');
...
app.post('/api/feedback', function (req, res) {
	...
	questions.forEach((question, i) => {
		...
		if(question["audioContent"] !== '') {
			...
			get_sentiments(sentimentAnalysisInputData, sentiment => {
				finalResult["questions"][i-1]["sentiment"] = sentiment;
				checkComplete();
				//
			});
		}
		...
		for (var i=0; i < finalResult["questions"].length; i++){
			var questionTempJson = {
				"user_id": username,
				"practice_id": interview_id,
				"feedback": JSON.stringify(finalResult["questions"][i])
			};
			foo("interviewData", questionTempJson);
		}
	...
	var interviewTempJson = {
		"user_id": username,
		"interview_id": interview_id,
		"expressions": JSON.stringify(finalResult["snapshots"])
	};

	foo("userInterviewExpressions", interviewTempJson);
